...
describe('Resilience and Hardening', () => {
    ...
    describe('GET /ready', () => {
        ...
        it('returns 503 UNAVAILABLE when DB is unreachable', async () => {
            ...
            prisma.$queryRaw = async () => { throw new Error('Connection failed') };
    ...
    describe('Rate Limiting', () => {
        it('returns 429 when login limit is exceeded', async () => {
            ...
            const res429 = await request(app).post('/auth/login').send({ email: 'test@example.com', password: 'password123' });
